...
// .wrangler/tmp/bundle-hsjuT3/checked-fetch.js
...
// .wrangler/tmp/bundle-hsjuT3/strip-cf-connecting-ip-header.js
...
var evidence = sqliteTable("evidence", {
  id: text("id").primaryKey(),
...
__name(CaseRepository, "CaseRepository");

// src/routes/cases.ts
var casesRoute = new Hono2();
casesRoute.get("/", async (c) => {
  const db = drizzle(c.env.DB);
  const repo = new CaseRepository(db);
  try {
    const cases2 = await repo.findAll();
    return c.json(cases2);
  } catch (error) {
...
// .wrangler/tmp/bundle-hsjuT3/middleware-insertion-facade.js
...
// .wrangler/tmp/bundle-hsjuT3/middleware-loader.entry.ts
